...
static std::atomic_size_t s_index((size_t)-1);
const static std::vector<std::string> s_codes =
{
...
util::strconv::strconv(util::strconv::code_t from, util::strconv::code_t to) : m_from(from), m_to(to)
{
}

std::string util::strconv::code(const std::string & src)
{
    return code(src.data(), src.size());
}

std::string util::strconv::code(const char* src, size_t size)
{
    std::string dest;

    if (m_from == m_to) {
        return dest.assign(src, size);
    }
    else if (m_from == strconv::code_t::mbs && m_to == strconv::code_t::utf16) {
        util::code_mbs_u16(src, size, dest);
    }
    else if (m_from == strconv::code_t::utf16 && m_to == strconv::code_t::mbs) {
        util::code_u16_mbs(src, size, dest);
    }
    else if (m_from == strconv::code_t::utf16 && m_to == strconv::code_t::utf8) {
        util::code_u16_u8(src, size, dest);
    }
    else if (m_from == strconv::code_t::utf8 && m_to == strconv::code_t::utf16) {
        util::code_u8_u16(src, size, dest);
    }
    else if (m_from == strconv::code_t::mbs && m_to == strconv::code_t::utf8) {
        util::code_mbs_u8(src, size, dest);
    }
    else if (m_from == strconv::code_t::utf8 && m_to == strconv::code_t::mbs) {
        util::code_u8_mbs(src, size, dest);
    }
...
void util::set_charset(const char* _charset)
{
    for (size_t i = 0; i < s_codes.size(); i++) {
        if (s_codes[i] == _charset) {
            s_index = i;
            return;
        }
    }

    throwx(invalid_argument, "_charset(%s):unexists", _charset);
}

bool util::code_chk_u8(const void* src, size_t size)
{
...
#if defined _WIN32
template<UINT _code_t, typename _from_t, typename _to_t, typename _func_t, typename ... _args_t>
inline void convto(_func_t conv, const void* src, size_t size, std::string & buf, _args_t ... args)
{
    size_t len = 0;

    len = conv(_code_t, 0, (const _from_t*)src, (int)size / sizeof(_from_t), nullptr, 0, args ...);
    if (len == 0) {
        throwx(runtime_error, "src(0x%p):size(%zu):len(%zu)", src, size, len);
    }

    buf.resize(len * sizeof(_to_t));

    len = conv(_code_t, 0, (const _from_t*)src, (int)size / sizeof(_from_t), (_to_t*)buf.data(), (int)len, args ...);
    if (len == 0) {
        throwx(runtime_error, "src(0x%p):size(%zu):len(%zu)", src, size, len);
    }
}
...
inline void convto(const char* from, const char* to, const void* src, size_t size, std::string & buf)
{
    size_t len = 0;

    iconv_t conv_id = iconv_open(to, from);
    if (conv_id == (iconv_t)-1) {
        throwx(runtime_error, "src(0x%p):size(%zu):conv_id(-1):from(%s):to(%s)", src, size, from, to);
    }

    do {
        buf.resize(len += size * sizeof(_type_t) + sizeof(_type_t));

        char* pbuf = (char*)buf.data();
        size_t plen = buf.size();
        if (iconv(conv_id, (char**)&src, (size_t*)&size, (char**)&pbuf, (size_t*)&plen) == (size_t)-1) {
            if (errno == E2BIG) {
                continue;
            }
            iconv_close(conv_id);

            throwx(runtime_error, "src(0x%p):size(%zu):plen(%zu):from(%s):to(%s)", src, size, plen, from, to);
        }
        else {
            buf.resize(len - plen);
            break;
...
void util::code_mbs_u16(const void* src, size_t size, std::string & dest)
{
    if (src == nullptr || size == 0) {
        throwx(invalid_argument, "src(0x%p):size(%zu)", src, size);
    }
...
#if defined _WIN32
    convto<CP_ACP, char, wchar_t>(::MultiByteToWideChar, src, size, dest);
#else
    const size_t index = s_index;
    const char* code = index == (size_t)-1 ? nl_langinfo(CODESET) : s_codes[index].c_str();
    convto<wchar_t>(code, "UCS-2", src, size, dest);
#endif
}

void util::code_u16_mbs(const void* src, size_t size, std::string & dest)
{
    if (src == nullptr || size == 0) {
        throwx(invalid_argument, "src(0x%p):size(%zu)", src, size);
    }
...
#if defined _WIN32
    convto<CP_ACP, wchar_t, char>(::WideCharToMultiByte, src, size, dest, nullptr, nullptr);
#else
    const size_t index = s_index;
    const char* code = index == (size_t)-1 ? nl_langinfo(CODESET) : s_codes[index].c_str();
    convto<char>("UCS-2", code, src, size, dest);
#endif
}

void util::code_u16_u8(const void* src, size_t size, std::string & dest)
{
    if (src == nullptr || size == 0) {
        throwx(invalid_argument, "src(0x%p):size(%zu)", src, size);
    }
...
#if defined _WIN32
    convto<CP_UTF8, wchar_t, char>(::WideCharToMultiByte, src, size, dest, nullptr, nullptr);
#else
    convto<char>("UCS-2", "UTF-8", src, size, dest);
#endif
}

void util::code_u8_u16(const void* src, size_t size, std::string & dest)
{
    if (src == nullptr || size == 0) {
        throwx(invalid_argument, "src(0x%p):size(%zu)", src, size);
    }
...
#if defined _WIN32
    convto<CP_UTF8, char, wchar_t>(::MultiByteToWideChar, src, size, dest);
#else
    convto<wchar_t>("UTF-8", "UCS-2", src, size, dest);
#endif
}

void util::code_mbs_u8(const void* src, size_t size, std::string & dest)
{
    if (src == nullptr || size == 0) {
        throwx(invalid_argument, "src(0x%p):size(%zu)", src, size);
    }

    if (code_chk_u8(src, size)) {
        dest.assign((const char*)src, size);
    }
    else {
#if defined _WIN32
        std::string temp;
        convto<CP_ACP, char, wchar_t>(::MultiByteToWideChar, src, size, temp);
        convto<CP_UTF8, wchar_t, char>(::WideCharToMultiByte, temp.data(), temp.size(), dest, nullptr, nullptr);
#else
        const size_t index = s_index;
        const char* code = index == (size_t)-1 ? nl_langinfo(CODESET) : s_codes[index].c_str();
        convto<wchar_t>(code, "UTF-8", src, size, dest);
#endif
    }
}

void util::code_u8_mbs(const void* src, size_t size, std::string & dest)
{
    if (src == nullptr || size == 0) {
        throwx(invalid_argument, "src(0x%p):size(%zu)", src, size);
    }
...
#if defined _WIN32
        std::string temp;
        convto<CP_UTF8, char, wchar_t>(::MultiByteToWideChar, src, size, temp);
        convto<CP_ACP, wchar_t, char>(::WideCharToMultiByte, temp.data(), temp.size(), dest, nullptr, nullptr);
#else
        const size_t index = s_index;
        const char* code = index == (size_t)-1 ? nl_langinfo(CODESET) : s_codes[index].c_str();
        convto<char>("UTF-8", code, src, size, dest);
#endif
    }
}
